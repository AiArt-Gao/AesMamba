...
class AesMamba_p(nn.Module):
    ...
    def __init__(self, device,type = 'NULL'):
        super(AesMamba_p, self).__init__()
        self.type = type

        if self.type == 'vmamba_base':
            self.img_model = VSSM(drop_path_rate = 0.,
                          depths=[ 2, 2, 15, 2 ],
                          ssm_d_state=1,
                          ssm_dt_rank="auto",
                          ssm_ratio=2.0,
                          ...
                          dims = 128,
                          forward_type = "v3noz")
            # d = torch.load('/data/sjq/IQA/Exp/vssm/vssm_base_0229_ckpt_epoch_237.pth', map_location='cpu')
    ...
    def forward(self, img, text):
        img_feature =self.proj(self.img_model(img)) 
...
class bert_feature(nn.Module):
    def __init__(self, device):
        ...
        # new 3090
        checkpoint = '/data/sjq/IQA/Exp/bert_base'
