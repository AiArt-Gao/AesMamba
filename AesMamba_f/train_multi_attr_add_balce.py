...
def start_train(opt):
    ...
    model = multi_attr_pred_model(type=opt.model_type).to(opt.device)
...
def start_check_model(opt):
    train_loader, test_loader = create_data_part(opt)
    model = multi_attr_pred_model(type=opt.model_type)
    model.eval()
    print(model.load_state_dict(torch.load(opt.pretrain_pth, map_location='cpu')))
